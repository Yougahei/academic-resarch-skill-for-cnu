...
- Guangxi undergraduate DOCX control rules: use A4 margins 2.54 cm top/bottom and 2.2 cm left/right; fixed 20 pt body line spacing; cover logo approximately 3.2 cm high × 10.6 cm wide; "本科毕业论文" in bold 宋体 小初 centered; topic/title in 黑体 一号; college/major/class/student/advisor fields and date in centered 宋体 四号; Chinese abstract title "摘　要" in 黑体 三号 centered with one blank line above/below; keywords use 黑体 小四 label and 宋体 小四 terms separated by "；"; English text uses Times New Roman; TOC title "目　录" and acknowledgements title "致　谢" use 黑体 三号 centered with character spacing; chapter headings use `第一章` / `第二章` and lower headings use multilevel numbering; figures/tables require bilingual captions; references use hanging indent two Chinese characters.
