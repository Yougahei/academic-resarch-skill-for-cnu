...
│   ├── postprocess_chinese_thesis_docx.py  # DOCX 后处理
...
关键契约：`experiment_intake_declaration` — fail-closed 的 Stage 1 声明，把"有没有跑实验"变成一个必须在上游就做出的明确决定。缺失此声明（在 post-#260 passport 上）即为 gate FAIL。即使纯文献流水线也必须携带 `{status: no_experiments_declared}`。见 `shared/handoff_schemas.md` §Experiment Provenance Intake。
...
关键契约：`ARS_SOCRATIC_READING_PROBE=1` 在目标导向 Socratic session 中触发一次性阅读诚实探测。用户引用特定论文时，Mentor 会请其摘述一段文本。拒绝回答仅记录，不扣分。默认关闭。见 `deep-research/agents/socratic_mentor_agent.md` §"Optional Reading Probe Layer"。
