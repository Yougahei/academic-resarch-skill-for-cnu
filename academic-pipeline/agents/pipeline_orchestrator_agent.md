...
| Stage 5 output process | Step 1: Produce MD -> Step 2: Select target style/profile -> Step 3: Generate DOCX via Pandoc when available (otherwise provide instructions) -> Step 4: Generate LaTeX with the selected template/profile when requested -> Step 5: User confirms content is correct -> Step 6: Produce PDF (final version) |
