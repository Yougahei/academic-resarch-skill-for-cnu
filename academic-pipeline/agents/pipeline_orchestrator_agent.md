...
| Stage 4.5 -> 5 | Final Verified Draft + Final Integrity Report | Schema 4 + Schema 5 (Integrity Report) | Preserve the original finalization path: produce MD -> DOCX via Pandoc when available (otherwise instructions) -> ask about LaTeX -> confirm -> PDF. If the user explicitly selects Chinese university DOCX final formatting, additionally call `scripts/docx_thesis_formatter.py` to produce a formatted DOCX copy + report. Carry forward `experiment_alignment_results[]` + `experiment_intake_declaration` (#260) to formatter surface + Stage 6 histogram |
...
| Stage 5 output process | Step 1: Produce MD -> Step 2: Generate DOCX via Pandoc when available (otherwise provide instructions) -> Step 3: if Chinese university DOCX final formatting is requested, generate a separate formatted DOCX copy + report via `scripts/docx_thesis_formatter.py` -> Step 4: Ask "Need LaTeX?" -> Step 5: User confirms content is correct -> Step 6: Produce PDF (final version) |
